                    ...
                    <div class="row">
                        <div class="col-lg-12 p-5">
                            ...
                            <?php if (isset($_GET['err_ckcode'])): ?>
                                <div class="alert alert-danger"><?= lang('validation_error_tip') ?></div><?php endif ?>
                            <?php if (isset($_GET['err_login'])): ?>
                                <div class="alert alert-danger"><?= lang('password_invalid') ?></div><?php endif ?>
                            <?php if (isset($_GET['err_forbid'])): ?>
                                <div class="alert alert-danger"><?= lang('account_forbid') ?></div><?php endif ?>
                            <form method="post" class="user" action="./account.php?action=dosignin&s=<?= $admin_path_code ?>">
                                <div class="form-group">
                                    <input type="text" class="form-control form-control-user" id="user" name="user" aria-describedby="emailHelp" placeholder="<?= lang('user_name_email') ?>"
                                        required
                                        autofocus>
                                </div>
                                <div class="form-group">
