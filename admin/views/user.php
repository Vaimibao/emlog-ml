...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h4 mb-0 text-gray-800 mt-3"><?= lang('user_management') ?></h1>
            ...
            <div class="form-inline">
                <div id="f_t_order" class="mx-1">
                    <select name="order" id="order" onChange="selectOrder(this);" class="form-control">
                        <option value="date" <?= (empty($order)) ? 'selected' : '' ?>><?= lang('recently_registered') ?></option>
                        <option value="update" <?= ($order === 'update') ? 'selected' : '' ?>><?= lang('recently_active') ?></option>
                        <option value="admin" <?= ($order === 'admin') ? 'selected' : '' ?>><?= lang('management_priority') ?></option>
                        <option value="forbid" <?= ($order === 'forbid') ? 'selected' : '' ?>><?= lang('forbidden_priority') ?></option>
                    </select>
                </div>
            </div>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if ($forbid): ?>
                                        <span class="badge badge-warning"><?= lang('banned') ?></span>
                                    <?php endif ?>
                                    <br />
                                    ...
                                    <span class="small mr-2"><?= lang('points') ?>:<?= $val['credits'] ?></span>
                                </td>
                                <td><?= $val['email'] ?></td>
                                <td><?= $val['ip'] ?></td>
        ...
        <div class="form-inline">
            <div class="btn-group">
                ...
                <div class="dropdown-menu">
                    <a href="javascript:useract('forbid');" class="dropdown-item text-warning"><?= lang('ban') ?></a>
                    <a href="javascript:useract('unforbid');" class="dropdown-item    "><?= lang('unban') ?></a>
                </div>
            </div>
        </div>
    </div>
...
<div class="d-flex justify-content-center mb-4 small">
    <form action="user.php" method="get" class="form-inline">
        <label for="perpage_num" class="mr-2"><?= lang('have') ?> <?= $userCount ?> <?= lang('_users_per_page') ?></label>
...
<script>
    ...
    function useract(act) {
        if (getChecked('ids') === false) {
            infoAlert('<?= lang('select_user') ?>');
            return;
        }

        if (act === 'forbid') {
            delAlert2('', '<?= lang('ban') . lang('_selected_users') ?>?', function() {
                    $("#operate").val("forbid");
                    $("#form_user").submit();
                },
                '<?= lang('ban') ?>')
        ...
        if (act === 'unforbid') {
            delAlert2('', '<?= lang('unban') . lang('_selected_users') ?>?', function() {
