...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h4 mb-0 text-gray-800 mt-3"><?= lang('template_manager') ?></h1>
    <div class="mt-3">
        <a href="store.php" class="btn btn-primary btn-sm shadow-sm mr-2"><i class="icofont-shopping-cart mr-1"></i><?= lang('app_store') ?></a>
        <a href="#" class="btn btn-success btn-sm shadow-sm" data-toggle="modal" data-target="#addModal"><i class="icofont-plus mr-1"></i><?= lang('template_add') ?></a>
    </div>
</div>
        ...
        <div class="col-md-6 col-lg-3 mb-4">
            <div class="card h-100 border-0 hover-shadow-lg" data-app-alias="<?= $value['tplfile'] ?>" data-app-version="<?= $value['version'] ?>">
                <div class="card-header border-0 pd-main <?php if ($nonce_template == $value['tplfile']) {
                    ...
                    <h5 class="card-title mb-0 font-weight-bold rem16"><?= $value['tplname'] ?></h5>
                </div>
                ...
                <div class="card-footer bg-white border-0">
                    ...
                    <p class="card-text text-muted small mb6">
                        <?= $value['tpldes'] ?>
                        <?php if (strpos($value['tplurl'], 'https://www.emlog.net') === 0): ?>
                            ｜ <a href="<?= $value['tplurl'] ?>" target="_blank"><?= lang('detailed_information') ?>&rarr;</a>
                        <?php endif ?>
                    </p>
                        ...
                        <div>
                            <?php if ($nonce_template !== $value['tplfile']): ?>
                                <a class="btn btn-outline-success btn-sm" href="template.php?action=use&tpl=<?= $value['tplfile'] ?>&token=<?= LoginAuth::genToken() ?>">
                                    <i class="icofont-check-circled mr-1"></i><?= lang('_enable') ?>
                                </a>
                            <?php else: ?>
            ...
            <form action="./template.php?action=upload_zip" method="post" enctype="multipart/form-data">
                <div class="modal-body px-4">
                    <div class="custom-file">
                        ...
                        <label class="custom-file-label" for="tplzip"><?= lang('select_template_package') ?></label>
                        <input name="token" value="<?= LoginAuth::genToken() ?>" type="hidden" />
                    </div>
                    <small class="form-text text-muted mt-2">
                        <?= lang('upload_template_package_tip') ?>
                    </small>
                </div>
...
<script>
    ...
    $(function() {
        ...
        // Monitor template file upload
        $('#tplzip').on('change', function() {
            var fileName = $(this).get(0).files[0] ? $(this).get(0).files[0].name : '';
            $(this).next('.custom-file-label').text(fileName || '<?= lang('select_template_package')?>');
        });
            ...
            success: function(response) {
                if (response.code === 0) {
                    ...
                    $.each(pluginsToUpdate, function(index, item) {
                        ...
                        var $updateLink = $('<a>').addClass('btn btn-warning btn-sm').text('<?= lang('update') ?>').attr("href", "javascript:void(0);");
                    ...
                    console.log(lang('update_api_error'));
                }
            },
            error: function() {
