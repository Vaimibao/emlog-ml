...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h4 mb-0 text-gray-800 mt-3"><?= lang('tag_management') ?></h1>
            ...
            <?php if ($tags): ?>
                ...
                ?>
                    <div class="badge badge-light m-3 p-2">
                        ...
                        <span class="<?= $count_style ?>">（<a href="./article.php?tagid=<?= $v['tid'] ?>" target="_blank"><?= $count ?><?= lang('_posts') ?></a>）</span>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-0">
                <h5 class="modal-title" id="exampleModalLabel"><?= lang('modify_tag') ?></h5>
            ...
            <form method="post" action="tag.php?action=update_tag">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="title"><?= lang('tag_page_title') ?></label>
                        <input type="text" class="form-control" id="title" name="title">
                        <small class="form-text text-muted"><?= lang('support_variables') ?>: {{site_title}}, {{site_name}}, {{tag_name}}</small>
                    </div>
                    <div class="form-group">
                        <label for="kw"><?= lang('tag_page_kw') ?></label>
                        <input type="text" class="form-control" id="kw" name="kw">
                    </div>
                    <div class="form-group">
                        <label for="description"><?= lang('tag_page_description') ?></label>
...
<script>
    ...
    function tagact(act) {
        if (getChecked('tids') === false) {
            infoAlert('<?= lang('tag_select_to_delete') ?>');
            return;
        }
