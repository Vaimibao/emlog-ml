...
<?php if (isset($_GET['error_a'])): ?>
    <div class="alert alert-danger"><?= lang('_name_empty') ?></div><?php endif ?>
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h4 mb-0 text-gray-800 mt-3"><?= lang('resource_library') ?></h1>
...
<?php if (User::isAdmin()): ?>
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <div>
            ...
            <?php endif ?>
            <a href="#" class="btn btn-light btn-sm my-1" data-toggle="modal" data-target="#mediaSortModal"><i class="icofont-plus"></i> <?= lang('categories') ?></a>
        </div>
            ...
            <div class="ml-2">
                <form action="./media.php" method="get" class="form-inline w-100">
                    <div class="input-group">
                        <input type="text" name="keyword" value="<?= $keyword ?>" class="form-control small" placeholder="<?= lang('search_resource_name') ?>">
                    ...
                    <div class="card mb-4 shadow-sm hover-shadow-lg">
                        ...
                        <div class="card-body">
                            <p class="card-text text-muted small">
                                ...
                                <?php if ($value['width'] && $value['height']): ?>
                                    , <?= lang('image_size') ?>：<?= $value['width'] ?>x<?= $value['height'] ?>
                                <?php endif ?><br>
                                <?= lang('source_file') ?>：<a href="#" class="copy-link text-muted" data-toggle="popover" data-url="<?= $media_url ?>"><?= $media_url ?></a><br>
                                <a href="#" class="copy-link" data-toggle="popover" data-url="<?= $media_url ?>"><?= lang('origin_file_path') ?></a>
                                ...
                                ?>
                                    ｜ <a href="#" class="copy-link" data-toggle="popover" data-url="<?= $media_down_url_pub ?>"><?= lang('show_public_download_address') ?></a>
                                    ｜ <a href="#" class="copy-link" data-toggle="popover" data-url="<?= $media_down_url ?>"><?= lang('user_download_address') ?></a> （<?= lang('_download') ?><?= $value['download_count'] ?>）
                                <?php endif ?>
                                <?php if ($thumbnail_url): ?>
                                    ｜ <a href="#" class="copy-link" data-toggle="popover" data-url="<?= $thumbnail_url ?>"><?= lang('thumbnail_address') ?></a>
                                <?php endif ?>
                            </p>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <form action="media.php?action=add_media_sort" method="post">
                ...
                <div class="modal-footer border-0">
                    <button type="button" class="btn btn-sm btn-light" data-dismiss="modal"><?= lang('save') ?></button>
...
<div class="modal fade" id="editMediaModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content border-0 shadow">
            <div class="modal-header border-0">
                <h5 class="modal-title" id="exampleModalLabel"><?= lang('edit_resource') ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form method="post" action="media.php?action=update_media">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="filename"><?= lang('resource_name') ?></label>
...
<script>
    ...
    $(function() {
        ...
        if (window.outerWidth > 767) {
            ...
            $('.highslide').removeAttr('onclick') // If it is a mobile terminal, the highslide function is not used
        }
    ...
    // Callback function for closing date selector
    ...
    function mediaact(act) {
        if (getChecked('aids') === false) {
            infoAlert('<?= lang('resource_select') ?>');
            return;
        }
